...
static int
start_fuse_process (WrapDir *wrapdirs,
                    int num_wrapdirs)
{
  char buf = 'x';
  int status_socket;
  ...
  for (int i = 0; i < num_wrapdirs; i++)
    {
      WrapDir *wrapdir = &wrapdirs[i];

      wrapdir->dirfd = openat (AT_FDCWD, wrapdir->path, O_RDONLY | O_NONBLOCK | O_DIRECTORY | O_CLOEXEC | O_NOCTTY);
      if (wrapdir->dirfd == -1)
        die_with_error ("Failed to open wrapped dir %s", wrapdir->path);
    }

  if (double_fork_with_socket (&status_socket) != 0)
    {
      for (int i = 0; i < num_wrapdirs; i++)
        close (steal_fd (&wrapdirs[i].dirfd));
      return status_socket;
    }
  ...
  for (int i = 0; i < num_wrapdirs; i++)
    {
      ...
      if (start_grootfs_lowlevel (wrapdir->dirfd, dev_fuse_fd, wrapdir->path) != 0)
...
main (int argc, char *argv[])
{
  ...
  if (conf.num_wrapdirs > 0)
    fuse_status_socket = start_fuse_process (conf.wrapdirs, conf.num_wrapdirs);
  ...
  if (conf.num_wrapdirs > 0)
    {
      for (int i = 0; i < conf.num_wrapdirs; i++)
        {
          WrapDir *wrapdir = &conf.wrapdirs[i];
          int dev_fuse_fd;

          dev_fuse_fd = mount_fuse_fd_at (wrapdir->path);
